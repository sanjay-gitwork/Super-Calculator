...
function CompoundInterestCalculator() {
  ...
  const calculateInterest = () => {
    if (validateInputs()) {
      ...
      const amount = principalValue * Math.pow(1 + rateValue / compoundFrequency, compoundFrequency * timeValue);
      const calculatedInterest = amount - principalValue;
  ...
  return (
    <div className="container mt-5 ">
      <div className="card shadow">
        <h1 className="card-header text-center">Compound Interest Calculator</h1>
        <div className="card-body">
            <p>Compound interest refers to the interest calculated on the initial principal and also on the accumulated interest of previous periods. It's the interest on a 
                loan or deposit that's calculated based on both the initial principal and the accumulated interest from previous periods.</p>
          <form>
          ...
          {interest > 0 && (
            <div className="mt-3 text-center">
              ...
              <p className="text-muted">
                Total Amount: {selectedCurrency} {parseFloat(principal) + parseFloat(interest)}
              </p>
            </div>
          )}
        </div>
